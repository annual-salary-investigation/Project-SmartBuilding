...
# RGB LED 모듈의 빨간색, 녹색에 대응하는 GPIO 핀 설정
red_pin = 6
green_pin = 5
...
# 초기 상태는 빨간불 켜진 상태로 설정
turn_off_green_led()
turn_on_red_led()

# 지정차량 입차시 초록불로 변경하는 코드
def change_to_green_led():
    turn_on_green_led()
    turn_off_red_led()
    ...
    while True:      
        ...
        if distance < 8 : # 8보다 작은 경우
            ...
            if (len(result_chars) == 7 or len(result_chars) == 8): # 추출된 글자가 자동차 번호판 글자수와 일치하면
                Clean() # 만차/잘못들어가 있는 데이터를 DB에서 삭제
                ...
                elif not registered_car and not parking_full: # 일반차량이 감지되고, 만차가 아닐 경우에도 서보모터 동작
                    ServoMotor(0)
